...
class GF2Mat(pandas.DataFrame):
    ...
    def get_solution_row(self, solution: list[int]) -> list[int]:
        ...
        """
        if (len(solution) != self.var_count):
            return []
    ...
    def xor_same_indices(self, drop: bool = True) -> pandas.DataFrame:
        ...
        for x in range(2, self.degree+1):
            for index in range(1, self.var_count+1):
                index =  (index, )
    ...
    def determine_additional_vars(self, stats: dict[str, int], assumptions: list, determined_vars: dict[str, bool], threshold: int=0) -> None:
        ...
        reversed_stats = self.find_keys_with_same_values(stats)
    ...
    def solve_linear(self, determined_vars: dict[str, bool]) -> pandas.DataFrame:
        ...
        """
        matrix = GF2Mat(self.degree, self.var_count, self.most_common, self.vars_needed,  self.copy())
        for key, value in determined_vars.items():
            var = int(re.search(r'\d+', key).group())
            matrix.set_variable(var, value)
        linear = GF2Mat(matrix.degree, matrix.var_count, matrix.most_common, matrix.vars_needed, matrix.get_linear_submatrix())
        linear = GF2Mat(linear.degree, linear.var_count, linear.most_common, linear.vars_needed, linear.galois_row_reduce())
        linear = GF2Mat(linear.degree, linear.var_count, linear.most_common, linear.vars_needed, linear.drop_empty_rows())
        if linear.shape[0] < linear.shape[1] - 1:
            return pandas.DataFrame()
        if linear.shape[0] == linear.shape[1] - 1:
            return linear.numpy_linalg_solve()
        
        linear = GF2Mat(linear.degree, linear.var_count, linear.most_common, linear.vars_needed, linear[:-(linear.shape[0] - (linear.shape[1]-1))])
        return linear.numpy_linalg_solve()
    ...
    def solve_combination(self, combination: tuple[bool, ...], solvers: list[Solver], contexts: dict[Solver, Context], threshold: int = 0) -> list[int]:
        ...
        while len(determined_vars) < vars_needed:
            ...
            stats = {f'x{i}': 0 for i in range(1, self.var_count+1)}
    ...
    def find_solution(self, group_size: int = 10, solver_count: int = 10, start_len: int = 1, skip: int = 0, threshold: int = 0) -> list[int]:
        ...
        for i in range(start_len, self.var_count+1):
    ...
    def check_solver(self, solver: Solver, assumptions: list, ctx: Context) -> bool:
        ...
        stats: dict[str, int] = {f'x{i}': 0 for i in range(1, self.var_count+1)}
        model = solver.model()
    ...
    def set_most_common(self) -> None:
        ...
        """
        most_common: dict[str, int] = {f'x{i}': 0 for i in range(1, self.var_count+1)}
    ...
    def set_vars_needed(self) -> None:
        ...
        opt.add(x <= self.var_count, x >= 0)
        opt.add(col_count - (((self.var_count - x + 1) * (self.var_count + x)) / 2) <= row_count - x)

        opt.maximize(x)

        opt.check()

        self.vars_needed = self.var_count - opt.model()[x].as_long()


    def find_keys_with_same_values(self, _dict: dict) -> dict:
